...
int main(int argc, char **argv) {
  ...
  // Initialize write machinery. This includes a pointer to the png write
  // context and a pointer to the image information
  ...
  // row at a time
  auto data = std::make_unique<png_byte[]>(3 * width * height);
  const auto col_stride = 3;  
  const auto row_stride = col_stride * width;

  for (auto i = 0U; i < height; ++i) {
    for (auto j = 0U; j < width; ++j) {
      data[col_stride * j + i * row_stride] = 255;
      data[col_stride * j + i * row_stride + 1] = 255 * (j / (2 * static_cast<float>(width)));
      data[col_stride * j + i * row_stride + 2]  = 255 * (j / static_cast<float>(width));
    }
    png_write_row(png_ptr, &data[i * row_stride]);    
  }
  ...
  // See:
  // http://refspecs.linuxbase.org/LSB_3.1.0/LSB-Desktop-generic/LSB-Desktop-generic/libpng12.png.destroy.write.struct.1.html
